...
{
    private Dialogou_Line[] angelLines =
    {
    ...
    private Dialogou_Line[] devilLines =
    {
    ...
    void Start()
    {
        Dialogou_Line randomLine;

        if (Random.value < 0.5f)
        {
            randomLine = angelLines[Random.Range(0, angelLines.Length)];
        }
        else
        {
            randomLine = devilLines[Random.Range(0, devilLines.Length)];
        }
